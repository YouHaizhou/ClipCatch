# Skill：Git 协作（后端 Agent）

## 触发时机
后端 Agent 需要进行 Git 操作时调用此 Skill。
...
git push -u origin feature/backend-[task-name]
```
...
bugfix/backend-[description]
refactor/backend-[description]

示例：
feature/backend-search-pagination
...
- ✅ 只修改 `apps/backend/src/` 下的文件
- ❌ 不修改 `apps/frontend/`、`apps/desktop/`、根目录配置文件
